...
public:
    ...
    inline m44 operator*(const m44& rhs) const
    {
    ...
    inline static m44 RotateX(float angle)
    { 
    ...
    inline static m44 RotateY(float angle)
    {
        float s = std::sin(angle); float c = std::cos(angle);
    ...
    inline static m44 RotateZ(float angle)
    {
        float s = std::sin(angle); float c = std::cos(angle);
        return m44(v4(  c, -s,  0,  0),
                   v4(  s,  c,  0,  0),
                   Z,
                   W);
    }
    ...
    inline static m44 Projection(float width, float height, float fov, float nearPlane, float farPlane)
    {
